package com.game.guild.view
{
    ...
    public class GuildBannerChangeNamePanel extends BaseBox
    {
        private var txt_oldName:TextField;
        private var txt_newName:TextField;
        ...
        public function GuildBannerChangeNamePanel()
        {
            ...
            loadDisplay("res/guildchangebannername.swf");
            return;
        }// end function

        override protected function displayReady() : void
        {
            initBaseBoxUI("guildBannerChangeName");
            this.initUI();
            super.displayReady();
            return;
        }// end function
        ...
        {
            this.txt_oldName = getDisplayChildByName("txt_oldName");
            this.txt_newName = getDisplayChildByName("txt_newName");
            this.txt_matrial = getDisplayChildByName("txt_matrial");
            this.yes_btn = new BaseButton(getDisplayChildByName("yes_btn"));
            this.yes_btn.setText(LanguageCfgObj.getInstance().getByIndex("10761"));
            this.no_btn = new BaseButton(getDisplayChildByName("no_btn"));
            this.no_btn.setText(LanguageCfgObj.getInstance().getByIndex("10762"));
        ...
        private function __click(event:MouseEvent = null) : void
        {
            switch(event.target)
            {
                case this.yes_btn:
                {
                    GuildControl.getInstance().reqGuildChangeBannerName(UserObj.getInstance().playerInfo.guildId, this.txt_newName.text);
                    break;
                }
                case this.no_btn:
                {
        ...
        private function update() : void
        {
            ...
            if (UILoaded)
            {
                this.txt_oldName.text = GuildData.getInstance().guildInfo.guildBanner;
                this.txt_newName.text = this.newName;
                _loc_1 = GuildData.getInstance().guildInfo.bannerLevel;
                _loc_2 = BannerCfgObj.getInstance().getGuildBannerinfoByLev(_loc_1);
                _loc_3 = _loc_2.changname.split("_");
                _loc_4 = _loc_3[0];
                _loc_5 = _loc_3[0];
