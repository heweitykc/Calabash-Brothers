package com.game.stalls.view
{
    ...
    public class StallsItemChangePanel extends BaseBox
    {
        ...
        {
            StallsControl.getInstance().downLoad(this._info.prop.itemId, this._info.prop.num);
            return;
        }// end function
        ...
        private function __confirmClick(event:MouseEvent) : void
        {
            if (this._special == 1)
            {
                StallsControl.getInstance().changeItem(this._info.prop.itemId, int(this._coinTxt.text), 0, int(this._countTxt.text), this.pos);
            }
            else if (this._special == 2)
            {
                StallsControl.getInstance().changeItem(this._info.prop.itemId, 0, int(this._coinTxt.text), int(this._countTxt.text), this.pos);
            }
            else
            {
                StallsControl.getInstance().changeItem(this._info.prop.itemId, int(this._coinTxt.text), int(this._goldTxt.text), int(this._countTxt.text), this.pos);
            }
            return;
        }// end function
