package com.game.auction.view
{
    ...
    public class CurrencyStallsPanel extends BaseBox
    {
        ...
        private function __confirmClick(event:MouseEvent) : void
        {
            ...
            {
                Global.popManager.cue(StringUtil.languageReplace(LanguageCfgObj.getInstance().getByIndex("11889"), [this.MIN_PRICE]));
                return;
            }
            var _loc_3:* = long.parselong(this._info.prop.itemModelId.toString());
            AuctionControl.getInstance().upload(_loc_3, int(this._coinPriceTxt.text), int(this._countTxt.text));
            this.close();
            return;
        }// end function
        ...
        private function __coinChange(event:Event = null) : void
        {
            ...
            if (_loc_3 > 0)
            {
                _loc_2 = Math.ceil(_loc_2 * Number((GlobalCfgObj.getInstance().getVipTex() / 100).toFixed(2)));
            }
            else
            {
                _loc_2 = Math.ceil(_loc_2 * Number((GlobalCfgObj.getInstance().getTex() / 100).toFixed(2)));
            }
            if (_loc_2 < 1)
            {
                _loc_2 = 1;
