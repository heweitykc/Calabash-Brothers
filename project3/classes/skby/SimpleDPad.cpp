...
SimpleDPad::SimpleDPad(void)
{
	...
	listener->onTouchesBegan = CC_CALLBACK_2(SimpleDPad::onTouchesBegin, this);
	listener->onTouchesMoved = CC_CALLBACK_2(SimpleDPad::onTouchesMove, this);
	listener->onTouchesEnded = CC_CALLBACK_2(SimpleDPad::onTouchesEnd, this);	
...
bool SimpleDPad::initWithFile(CCString *filename, float radius)
{
	...
	do 
	{
		...
		auto bg = Sprite::create(filename->getCString());
		this->addChild(bg);
		_radius = radius;
...
void SimpleDPad::onTouchesBegin(const std::vector<Touch*>& touches, Event* event)
{
	CCPoint location = touches[0]->getLocation();
...
void SimpleDPad::onTouchesMove(const std::vector<Touch*>& touches, Event* event)
{
	CCPoint location = touches[0]->getLocation();
	this->updateDirectionForTouchLocation(location);
}

void SimpleDPad::onTouchesEnd(const std::vector<Touch*>& touches, Event* event)
{
	_direction = CCPointZero;
	_isHeld = false;
